...
def load_container():
    ...
    cont1_properties = table.dispense_container(3,True) #dispensing container 1 and assigning each property to a variable
...
def transfer_container(cont1_properties):
    ...
    while True: # setting a while loop for the motion of the Q bot
        ...
        if bin_number == "Bin01":
            if (sensor_value > 0.03 and sensor_value < 0.09):
                if (bot_position[0] > 1 and bot_position[0] < 1.1 and (bot_position[1] > 0.663 and bot_position[1] < 0.8)):
                    print("we are going to bin 1")
                    bot.stop()
                    break
        if bin_number == "Bin02":
            if (sensor_value > 0.16 and sensor_value < 0.2):
                if (bot_position[0] > 0 and bot_position[0] < 0.1 and (bot_position[1] > 0.663 and bot_position[1] < 0.8)):
...
def return_home():
    ...
    while True: # setting a while loop for the motion of the Q bot
        bot.set_wheel_speed([l_speed, r_speed])
        ir_values = bot.line_following_sensors()
        if bot.position() == (1.5, -2.39e-5, 0.00075):
            bot.stop()
